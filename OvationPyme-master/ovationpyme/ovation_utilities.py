...
def read_solarwind(dt,oi):
    ...
    swvars = OrderedDict(Bx='BX_GSE',
                         ...
                         V=velvar,
                         Ni=densvar)
    #add one hour or delta t to the original datetime measurements? omni data might be fine
    #could likely switch to ace data, however the interpreation is that that will be the aurora
    #one hour ahead of time, so historic runs won't work 
    ...
    #         hour=df['hhmm'] // 100,
    #         minute=df['hhmm'] % 100,
    #     )  # optional but recommended for solar wind data
    # )
    # minutes = 36 * 60  # 2160 minutes

    # dt_list = [
    #     dt + timedelta(minutes=m)
    #     for m in range(-minutes, minutes)
    # ]
    
    # mask = ((df['datetime'] - dt).abs() <= timedelta(hours=36))
    ...
    # plt.plot(np.arange(len(sw['V'])),sw['V'])
    # plt.plot(np.arange(len(df.vel[mask])), df.vel[mask])
    return sw
...
@cache_omni_interval('1min')
def calc_avg_solarwind(dt,oi, original = False, ace_input_avg = None):
    """
